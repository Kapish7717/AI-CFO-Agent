...
def rank_tables(query: str, table_specs: list, top_n: int = 0) -> list:
    ...
    try:
        resp = requests.post(RERANK_URL, headers=headers, json=data, timeout=60)
...
def _read_only(sql: str) -> bool:
    ...
    if not lower.startswith(("select", "with")):
        return False
    # Split on ';' — only a single trailing terminator is acceptable.
    parts = cleaned.split(";")
    if any(p.strip() and i != len(parts) - 1 for i, p in enumerate(parts)):
        return False
...
async def answer_with_rag(user_id: int, question: str) -> str:
    ...
    conn = get_connection()
    try:
        table_specs = extract_schema(conn)
        if not table_specs:
            return "No database tables available to query."
        ranked = rank_tables(question, table_specs, top_n=3)
        ...
        result = sql_response(sql_clean, conn)

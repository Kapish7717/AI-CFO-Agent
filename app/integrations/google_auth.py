...
def get_db_token(user_id: int):
    ...
    try:
        from app.db.database import get_user_google_token
...
def save_db_token(user_id: int, token_json_str: str):
    ...
    try:
        from app.db.database import save_user_google_token
...
def get_google_credentials(user_id: int = None):
    ...
    try:
        ...
        raise Exception(f"AUTH_REQUIRED: Local browser flow failed. {e}") from e
